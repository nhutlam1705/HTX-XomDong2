...
<div class="container mt-5">
    <div class="row">
        <div class="col-lg-8 col-xs-12 mt-2">
          <div class="row">
            <div class="news-event">Giới thiệu</div>
            <div class="card-title-line"></div>
          </div>
          @foreach($introduction as $introduce)
            <div class="row">
                <div class="card-tintuc col-12">
                  <a href="{{ url('pages/tintuc/chitiettintuc', $introduce->id) }}">
                      @if($introduce->image)
                        <img  class="card-img-top" src="{{ asset('storage/' . $introduce->image) }}" alt="{{ $introduce->image}}">
                      @endif
                  </a>
                      <div class="card-body">
                        <h5 class="card-title">{{ $introduce->title }}</h5>
                        <p class="card-text">{!! Str::limit(strip_tags($introduce->description), 100, '...') !!}</p>
                        <i class="fa-solid fa-calendar-days card-time-top-right">
                          {{ $introduce->created_at->format('d/m/Y') }}
                        </i>
                      </div>
                </div>
            </div>
